...
├── _client.py              → AttioClient / AsyncAttioClient (26 resource properties)
...
│   └── 18 standalone       → objects, lists, attributes, notes, tasks, files, webhooks, etc.
├── models/ (21 Pydantic)   → Record, Entry, Attribute, Object, List, etc.
...
- **Async Python classes are code-generated** from their sync counterparts by `scripts/generate_async.py`, covering all 26 resource files. This eliminates manual sync/async duplication.
...
The test suite covers 1,282 tests across the three packages:

```bash
# Python SDK (814 tests)
