...
- **Automatic retries** -- rate limits, 5xx errors, and transient failures are handled with exponential backoff per a [shared retry contract](docs/http-retry-spec.md)
- **Type safety** -- Pydantic models in Python, TypeScript interfaces in Node, structured output in the CLI
- **Auto-pagination** -- `query_all()` in Python, `queryAll()`/`listAll()` in Node (AsyncIterable), `--all` in the CLI
...
The CLI supports 25 command groups. Run `attio --help` to see them all.
...
# Search across all records (beta, fuzzy)
results = client.records.global_search(
    query="Acme", objects=["companies", "deals"], limit=10
)
...
Records and entries support `queryAll()` and `listAll()`, which return `AsyncIterable` streams that handle pagination automatically. Use `collectAll()` when you need everything in an array.

```typescript
import { collectAll } from 'attio-node';

// Stream records one at a time (memory-efficient)
for await (const person of client.records.queryAll('people', {
  filter: { email_addresses: { is_not_empty: true } },
})) {
  console.log(person.id.record_id);
}

// Or collect all into an array
const allDeals = await collectAll(client.records.queryAll('deals'));
```
...
| `client.files` (beta) | list, get, create_folder, connect, upload, download, delete, list_all |
| `client.meetings` (beta) | list, get, create (alpha), list_all |
| `client.call_recordings` (beta) | list, get, create (alpha), delete (alpha), list_all |
| `client.transcripts` (beta) | get, get_all |
| `client.sql` (beta, Enterprise) | query |

Record and entry `query` / `query_all` accept either a `filter` or a `filter_view_id`
(a saved view's ID from `client.views`) -- the two are mutually exclusive.
`client.records.global_search` is the beta fuzzy search endpoint
(`POST /v2/objects/records/search`) and supports `request_as` member impersonation.
...
The three packages share a consistent structure. This map is useful if you are contributing or debugging.

```
Python SDK (packages/sdk/)
├── _client.py              → AttioClient / AsyncAttioClient (27 resource properties)
├── _http.py                → HttpTransport (retry, rate-limit, timeout)
├── _queryable.py           → Shared CRUD base for Records + Entries
├── resources/
│   ├── records.py          → extends QueryableResource + list_entries, global_search
│   ├── entries.py          → extends QueryableResource
│   ├── _standard_object.py → convenience wrappers (people, companies, deals, users, workspaces)
│   └── 19 standalone       → objects, lists, attributes, notes, tasks, files, sql, webhooks, etc.
├── models/ (22 Pydantic)   → Record, Entry, Attribute, Object, List, etc.
├── _pagination.py          → Offset/Cursor iterators (sync + async)
└── scripts/generate_async.py → codegen: sync → async variants

CLI (packages/cli/)
├── main.py                 → Typer root, 25 command groups
├── _record_factory.py      → generates CRUD commands from object slug
├── _record_helpers.py      → record display, dynamic columns, value flattening
├── _client.py              → API key resolution (flag > env > config > 1Password)
└── commands/ (21 files)    → entries, notes, tasks, files, webhooks, etc.

Node SDK (packages/node-sdk/)
├── index.ts                → AttioClient, 17 resource properties
├── client.ts               → HttpClient (native fetch, retry, rate-limit)
├── pagination.ts           → paginateOffset, paginateCursor, collectAll (AsyncIterable)
├── types.ts                → hand-written TypeScript interfaces
└── resources/ (17 files)   → records, entries, + 15 more
```

**Key patterns:**

- **Records and Entries share a `QueryableResource` base class** (`_queryable.py`) that provides list, query, get, create, update, append, delete, upsert, get_attribute_values, and query_all. Each subclass only adds its unique methods.
- **CLI commands for standard objects are generated by a factory** (`_record_factory.py`). `people.py`, `companies.py`, and `deals.py` are each under 10 lines. Custom objects get dynamic column discovery via attribute metadata; standard objects use hardcoded columns for speed.
- **Async Python classes are code-generated** from their sync counterparts by `scripts/generate_async.py`, covering all 27 resource files. This eliminates manual sync/async duplication.
- **HTTP retry behavior** is defined in [docs/http-retry-spec.md](docs/http-retry-spec.md), which both SDKs implement: exponential backoff with jitter, `Retry-After` header support, and retries on 429 + 5xx.

---

## Testing

The test suite covers 1,338 tests across the three packages:

```bash
# Python SDK (870 tests)
...
All tests run against mocked HTTP responses -- no Attio API key required.
