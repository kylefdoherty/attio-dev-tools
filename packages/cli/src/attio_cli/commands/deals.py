...
def list_deals(
    ...
    try:
        if all_results:
            ...
            iterator = client.deals.query_all(filter=filter_obj)
            all_data = [record_to_dict(r) for r in iterator]
            output_list(all_data, DEALS_COLUMNS, ctx, title="Deals (all)")
            return
...
def create(
    ...
    try:
        ...
        values_obj = expand_values(client, "deals", values_obj)
