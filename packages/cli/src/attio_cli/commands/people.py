...
def list_people(
    ...
    try:
        if all_results:
            ...
            iterator = client.people.query_all(filter=filter_obj)
            all_data = [record_to_dict(r) for r in iterator]
            output_list(all_data, PEOPLE_COLUMNS, ctx, title="People (all)")
            return
...
def create(
    ...
    try:
        ...
        values_obj = expand_values(client, "people", values_obj)
